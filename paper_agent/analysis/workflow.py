...
from paper_agent.reporting.sections import detect_sections, select_experiment_sections
...
from .report_stage import run_report_render_stage
from .url_enrichment import (
    normalize_resource_payload,
)
from .url_resource_stage import collect_url_resource_candidates, run_url_resource_enrichment_stage
from .web_search import build_search_queries
from .workflow_support import (
    chat_analysis_text_with_fallback,
    empty_resource_discovery_payload,
    empty_web_research_payload,
    merge_resource_discovery_with_paper_signals,
    merge_web_research_with_paper_signals,
    pick_section_targets,
)
...
class PaperAnalysisWorkflow:
    ...
    def web_research(self, state: PaperState) -> PaperState:
        stage = "web_research"
        run_dir = Path(state["run_dir"])
        empty_payload = empty_web_research_payload()
        ...
        try:
            ...
            summary = merge_web_research_with_paper_signals(summary, state.get("paper_web_signals") or {})
            meta["enabled"] = True
    ...
    def resource_discovery(self, state: PaperState) -> PaperState:
        ...
        run_dir = Path(state["run_dir"])
        empty_payload = empty_resource_discovery_payload()
        ...
        try:
            ...
            resource_discovery = merge_resource_discovery_with_paper_signals(
                resource_discovery,
                state.get("paper_web_signals") or {},
            )
    ...
    def url_resource_enrichment(self, state: PaperState) -> PaperState:
        ...
        run_dir = Path(state["run_dir"])
        candidates = collect_url_resource_candidates(state, self.config)
        self._stage_start(
            state,
            stage,
        ...
        try:
            result, finish_fields = run_url_resource_enrichment_stage(
                config=self.config,
                state=state,
                run_dir=run_dir,
                stage=stage,
            ...
            self._stage_finish(state, stage, **finish_fields)
            return result
        except Exception as exc:
            self._stage_error(state, stage, exc, candidate_count=len(candidates))
            raise
    ...
    def structure_breakdown(self, state: PaperState) -> PaperState:
        ...
        try:
            ...
            selected_sections = pick_section_targets(
                structure,
    ...
    def critique(self, state: PaperState) -> PaperState:
        ...
        try:
            ...
            critique_markdown, meta = chat_analysis_text_with_fallback(
                client=self.client,
                config=self.config,
    ...
    def extensions(self, state: PaperState) -> PaperState:
        ...
        try:
            ...
            extensions_markdown, meta = chat_analysis_text_with_fallback(
                client=self.client,
                config=self.config,
                run_dir=state["run_dir"],
                web_search_enabled=state.get("web_search_enabled", False),
    ...
    def render_report_node(self, state: PaperState) -> PaperState:
        ...
        self._stage_start(state, stage)
        try:
            result, finish_fields = run_report_render_stage(
                config=self.config,
                state=state,
                run_dir=run_dir,
                stage=stage,
