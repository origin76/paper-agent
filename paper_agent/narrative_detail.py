...
from paper_agent.narrative import PaperProfile, ReadingStep, StoryArc, TurningPoint, YearMoment
...
from paper_agent.utils import sanitize_filename, slugify, write_json, write_text
...
def _render_single_arc_markdown(arc: DetailedStoryArc, bundle: ArcEvidenceBundle) -> str:
    ...
    if bundle.tensions:
        lines.append(
            f"一句话抓住这条线的张力：{_trim_text(bundle.tensions[0], max_chars=180)}"
        )
        lines.append("")
    ...
    if arc.reading_path_detailed:
        ...
        for index, item in enumerate(arc.reading_path_detailed, start=1):
            ...
            lines.append("")
            stage_prefix = f"{item.stage_label}。"
            lines.append(f"{stage_prefix}{item.why_read_now}")
            lines.append("")
    ...
    if bundle.selected_papers:
        ...
        lines.append("")
        lines.append(
            "下面这些卡片不是替代原论文，而是告诉你每篇在这条故事线里扮演什么角色、该抓什么证据、又该怀疑什么地方。"
        )
        lines.append("")
    ...
    lines.append("")
    lines.append(
        f"这份单独报告覆盖原始故事线中的 {arc.paper_count} 篇论文，并额外挑出 {len(bundle.selected_papers)} 篇代表论文作为锚点卡片。"
    )
    lines.append("")
    if bundle.selected_papers:
    ...
    lines.append("")
    lines.append(
        "这份单独故事线 PDF 复用了已有单篇解析产物与 narrative detail 中间结果，没有重新解析 PDF 原文；它的目标是把分散证据重新组织成适合连续阅读的一条研究脉络。"
    )
    lines.append("")
    return "\n".join(lines).strip() + "\n"


def _export_single_arc_reports(
    ...
    for arc in detailed_arcs:
        ...
        arc_markdown = _render_single_arc_markdown(arc, bundle)
